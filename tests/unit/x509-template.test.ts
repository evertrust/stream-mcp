...
describe('registerX509TemplateTools', () => {
  ...
  it('create_template POSTs the full camelCase wire body', async () => {
    ...
    // exact camelCase wire mapping + duration passthrough
    expect(body).toEqual({
      name: 'my-tls-server',
      lifetime: '365d',
      enabled: true,
  ...
  it('update_template GET-strips id, merges overrides (camelCase), PUTs to collection root', async () => {
    const { client, byName } = setup();
    // current record as read back (id present, duration in human form)
    ...
    expect(client.get).toHaveBeenCalledWith('/api/v1/templates/example-sub-ca');
    ...
    expect(putBody.name).toBe('example-sub-ca');
