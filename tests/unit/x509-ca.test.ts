...
describe('enum wire values', () => {
  it('QC_TYPES are the exact accepted values (no WEB_AUTHENTICATION; NONE present)', () => {
    // WEB_AUTHENTICATION is rejected by the server with CA-002.
    expect([...QC_TYPES]).toEqual(['WEB', 'ESIGN', 'ESEAL', 'NONE']);
    expect(QC_TYPES).not.toContain('WEB_AUTHENTICATION');
  });

  it('HASH_ALGORITHMS use underscore SHA-3 names (no hyphens)', () => {
    // SHA3-512 is rejected by the server with CA-002; SHA3_512 round-trips.
    // The server matches the exact name with no normalization.
