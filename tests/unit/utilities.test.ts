...
describe('decoders', () => {
  ...
  it('extract_pkcs12 POSTs multipart with file "pkcs12" + text part "password"', async () => {
    ...
    // Plain data field (no filename): the server reads the password as a form
    // field; a file part is not read as one.
...
describe('ekus', () => {
  ...
  it('create_eku POSTs flat { name, oid, custom:true } (custom mandatory on the wire)', async () => {
    ...
    // The server requires `custom` to be present (no default) —
  ...
  it('update_eku PUTs flat { oid, name, custom:true } on the collection root (no GET, no path param)', async () => {
    ...
    // `custom` is mandatory on the wire (the server has no default);
