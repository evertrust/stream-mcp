...
describe('create_ssh_ca', () => {
  ...
  it('maps nested override/triggers/krlPolicy + usePSS to camelCase', async () => {
    const { client, byName } = setup();
    client.post.mockResolvedValue({ name: 'example-ssh-ca' });
    await byName('create_ssh_ca').h({
      name: 'example-ssh-ca',
      private_key: { keystore: 'SSH', name: 'example-ssh-ca', use_pss: true },
      enroll: true,
...
describe('update_ssh_ca', () => {
  it('GET-strip-merge-PUTs: strips id/publicKey, PUTs collection root', async () => {
    ...
    expect(client.get).toHaveBeenCalledWith('/api/v1/ssh/cas/example-ssh-ca');
    ...
    expect(putBody.name).toBe('example-ssh-ca');
...
describe('generate_krl', () => {
  it('GETs :name/krl with no lazy param by default', async () => {
    ...
    await byName('generate_krl').h({ name: 'example-ssh-ca' });
    expect(client.get).toHaveBeenCalledWith(
      '/api/v1/ssh/cas/example-ssh-ca/krl',
      undefined,
    );
  });
  ...
  it('GETs :name/krl?lazy=true when lazy=true', async () => {
    ...
    await byName('generate_krl').h({ name: 'example-ssh-ca', lazy: true });
    const [path, params] = client.get.mock.calls[0]!;
    expect(path).toBe('/api/v1/ssh/cas/example-ssh-ca/krl');
...
describe('list_krls / get_krl', () => {
  ...
  it('get_krl GETs /ssh/krls/:ca with the encoded ca', async () => {
    const { client, byName } = setup();
    client.get.mockResolvedValue({ ca: 'example-ssh-ca', number: 2048 });
...
describe('revoke_ssh_certificate', () => {
  ...
  it('Variant B: serial + ca (with expected_serial confirmation)', async () => {
    ...
    const body = client.post.mock.calls[0]![1] as Record<string, unknown>;
    expect(body).toEqual({ serial: '12345', ca: 'example-ssh-ca' });
  });
  ...
  it('rejects when neither certificate nor serial+ca is provided', async () => {
    const { client, byName } = setup();
    const out = await textOf(
      byName('revoke_ssh_certificate').h({ ca: 'example-ssh-ca' }),
    );
