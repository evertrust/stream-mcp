...
describe('CRL tools', () => {
  it('list_crls uses getList on the collection and filters by ca', async () => {
    ...
    client.getList.mockResolvedValue([
      { ca: 'example-ca', type: 'managed', size: 0 },
    ...
    expect(body.items[0].ca).toBe('example-ca');
  });

  it('get_crl GETs /crls/:ca with the encoded ca segment', async () => {
    const { client, invoke } = setup();
    client.get.mockResolvedValue({ ca: 'example-ca', number: 298 });
    await invoke('get_crl', { ca: 'example-ca' });
    expect(client.get).toHaveBeenCalledWith('/api/v1/crls/example-ca');
  });

  it('update_crl_next_refresh PUTs {nextRefresh} keyed by :ca', async () => {
    const { client, invoke } = setup();
    client.put.mockResolvedValue({ ca: 'example-ca', nextRefresh: FUTURE_ISO });
    const result = await invoke('update_crl_next_refresh', {
      ca: 'example-ca',
      next_refresh: FUTURE_ISO,
    });
    expect(client.put).toHaveBeenCalledWith('/api/v1/crls/example-ca', {
      nextRefresh: FUTURE_ISO,
    });
    const body = parseText(result);
    expect(body.status).toBe('updated');
    expect(body.kind).toBe('crl');
    expect(body.name).toBe('example-ca');
  });
...
describe('OCSP signer CRUD', () => {
  ...
  it('update_ocsp_signer does GET-strip-merge-PUT stripping id/certificate but PRESERVING dn', async () => {
    ...
    client.get.mockResolvedValue({
      ...
      dn: 'CN=old', // NOT stripped (mandatory for cert-less signers; server clears it when a cert exists)
...
describe('CSR + CA assignment', () => {
  ...
  it('assign_ocsp_signer_to_ca GETs the CA, strips only server-managed fields, PRESERVES privateKey+dn, PUTs enableOCSP+ocspSigner', async () => {
    ...
    expect(client.get).toHaveBeenCalledWith('/api/v1/cas/example-ca');
    ...
    // "/privateKey: error.path.missing". The server keeps the previous
    // privateKey / clears dn for a certificated CA.
    expect(putBody.privateKey).toEqual({ keystore: 'KS', name: 'K' });
    expect(putBody.dn).toBe('CN=example-ca');
    // benign fields preserved
    expect(putBody.name).toBe('example-ca');
