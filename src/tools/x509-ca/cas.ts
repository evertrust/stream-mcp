...
const KNOWLEDGE_REF = 'stream://knowledge/ca-management';

const text = (s: string) => ({ content: [{ type: 'text' as const, text: s }] });

/**
 * Server-managed fields that are OPTIONAL in the request body, so they
 * can be dropped from the update PUT body entirely. The server resets them
 * (on update it clears revoked/revocationDate/revocationReason and takes id
 * from the previous record) — never author them.
 *
 ...
 * (the server validates the body BEFORE it merges it with the stored CA;
 * managed `privateKey` has no default, external `certificate` is required).
 * They are carried over from the GET in update_ca (certificate is
 * converted from the rich-on-read object to its PEM string, which is the only
 * shape the server accepts). `dn` is also carried over from the GET so a PENDING
 * managed CA (no cert) keeps its mandatory dn; an issued CA's GET has dn=null.
 */
/**
 * Server-managed / revocation fields that Stream resets on any
...
export function registerCaCrudTools(
  ...
  registerTool(
    ...
    async ({ config }) => {
      ...
      //    revocationReason) — they have no JSON default and the server
      //    resets them on update anyway;
      //  - convert the rich-on-read `certificate` object to its PEM string (the
      //    only shape the server accepts); keep `privateKey`/`altPrivateKey`/`dn`
