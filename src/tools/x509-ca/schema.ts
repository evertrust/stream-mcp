 ...
 *
 * Managed and external field sets are disjoint beyond the common
 * fields; both are surfaced as an optional superset on the typed `config`
 * input and validated against type on submit.
 */
import { z } from 'zod';
...
const duration = z
  .string()
  .regex(DURATION_RE, 'Must be a duration like "28 days" or "10 minutes".');
    ...
    hashAlgorithm: z
      ...
      .describe(
        `Signature hash; one of ${HASH_ALGORITHMS.join(' | ')}. ` +
          ...
          'algorithm" otherwise); omit only for EC/EdDSA keys.',
      ),
    usePSS: z
      .boolean()
      .optional()
...
// ---------------------------------------------------------------------------
// JSON Schema doc surfaced by describe_ca_schema (hand-authored).
// ---------------------------------------------------------------------------
export const CA_JSON_SCHEMA = {
  $comment:
    'Polymorphic CA body. `type` selects the field set; managed and external ' +
    'fields are disjoint beyond the common fields. Send certificate as a PEM ' +
