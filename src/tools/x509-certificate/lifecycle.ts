...
const EKU_ELEMENT_SCHEMA = z
  .object({
    ...
    // `custom` is REQUIRED on the wire (omitting it -> 400 path.missing).
    custom: z.boolean(),
  })
  .strict();
...
const TEMPLATE_OVERRIDES_SCHEMA = z
  .object({
    ...
    lifetime: z
      .string()
      .optional()
      .describe('Duration string, e.g. "365 days".'),
    backdate: z.string().optional().describe('Duration string, e.g. "1 hour".'),
