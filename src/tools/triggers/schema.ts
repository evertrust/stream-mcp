...
export const triggerInputSchema = z.object({
  type: z
    ...
    .describe(
      'Discriminator: email | rest. (`external_rl_storage` triggers are not ' +
...
/**
 * Enforce the server-side trigger invariants before any HTTP call:
 *   - type ∈ {email, rest} (external_rl_storage rejected)
...
export function validateTrigger(args: TriggerInput): void {
  if (!(NOTIFICATION_TYPES as readonly string[]).includes(args.type)) {
    throw new StreamError(422, {
      ...
      remediation:
        '`external_rl_storage` triggers are not supported by these tools.',
    });
  }
