/**
 * HTTP proxy CRUD (/api/v1/system/proxies routes). Standard name-keyed scaffold:
 * PUT-on-collection-root full-replace, name immutable, 204->[] on list.
 *
 * Body: { id (server), name (PK), host, port }.
 */
import { z } from 'zod';
...
// or IPv6 / CIDR. Mirrors the server-side validation rules so
