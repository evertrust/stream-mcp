 ...
 *
 * Endpoints:
 *   - GET /licenses          -> license info object
 *   - GET /licenses/modules  -> string[] of entitled module names (204 -> [])
...
export function registerSystemInfoTools(
  ...
  registerTool(
    ...
    {
      description:
        'Get the list of entitled Stream module names (e.g. stream-ca, stream-va, stream-tsa, stream-ssh) in declaration order. No permission required. Empty list if none entitled.\n' +
        'Safety tier: read-only',
      inputSchema: z.object({}),
    },
    async () => {
  ...
  registerTool(
    ...
    {
      description:
        'Get the supported asymmetric key types as objects { name, pqc, type }. The set is license/version-dependent — always read it live rather than assuming.\n' +
        'Safety tier: read-only',
      inputSchema: z.object({}),
    },
    async () => {
