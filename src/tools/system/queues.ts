/**
 * Queue CRUD (/api/v1/queues routes). Standard name-keyed scaffold:
 * PUT-on-collection-root full-replace, name immutable, 204->[] on list.
 *
 * Body: { id (server), name (PK), description?, size>0,
 *         throttleDuration? (duration string), throttleParallelism?>0,
...
// Duration wire format, e.g. "5 seconds" (matches the server regex,
...
const createSchema = z.object({
  ...
  throttle_duration: z
    ...
    .optional()
    .describe(
      'Optional throttle window as a duration string, e.g. "1 second", "500 ms", "5 minutes". If set, throttle_parallelism is required.',
    ),
  ...
  if (duration !== undefined) {
    if (!DURATION_RE.test(duration.trim())) {
      ...
      return (
        ...
        'valid duration. Use a value like "5 seconds", "500 ms", or ' +
        '"1 minute".'
      );
    }
