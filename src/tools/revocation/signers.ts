 ...
 *   POST   /api/v1/ocsp/signers          -> create (name unique; cert forced empty)
...
//     validated BEFORE the server merges it with the stored CA, so a
//     missing privateKey fails JSON validation with CA-002
//     "/privateKey: error.path.missing" (on both a certificated CA and a
//     managed-pending CA). For a certificated CA the server then keeps the
...
//     invariant. For a certificated CA the GET omits dn and the server
//     clears it, so keeping it out of the strip set is harmless either way.
//
// Only genuinely server-managed / rich-on-read fields are stripped:
//   - certificate: rich object on read / write-only PEM; on update the server
//     keeps the previous certificate regardless of the body.
//   - altPrivateKey: optional; server keeps the previous for a certificated CA.
//   - revoked/revocationDate/revocationReason: server-managed; the server
//     clears them on update.
...
// the PUT body BEFORE it merges it with the stored CA, and an ISSUED CA's GET
// has no dn - so a body with neither dn nor certificate fails CA-002
// "dn is mandatory when certificate is not specified".
  ...
  hash_algorithm: z
    ...
    .describe(
      'Signing hash algorithm. Allowed: SHA1, SHA224, SHA256, SHA384, ' +
        ...
        'hash algorithm" otherwise). Omit for EdDSA curves. ' +
...
export function registerSignerTools(
  ...
  registerCreateTool(server, client, SIGNER_SPEC, {
    ...
    // dn is mandatory on create: certificate is forced empty, and the server
    ...
    inputSchema: z.object({
      ...
      on_expiration_triggers: z
        ...
        .optional()
        .describe(
          'OPTIONAL. Trigger names fired on the on_ocsp_signer_expiration event. ' +
            'Each must reference an existing trigger.',
        ),
    }),
    buildPayload: (args) => {
