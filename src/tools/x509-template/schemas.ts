 ...
 * exact camelCase wire fields and drops any undefined optional so omitted
 * fields are simply absent from the body.
 */
import { z } from 'zod';
...
// ---------------------------------------------------------------------------
// Duration strings (lifetime, backdate)
// ---------------------------------------------------------------------------
...
const durationSchema = z
  ...
  .regex(
    DURATION_RE,
    'Duration like "365d" / "365 days" / "20m" / "5 minutes".',
  );
...
export const templateOptionalShape = {
  ...
  backdate: durationSchema
    .optional()
    .describe('notBefore backdating duration (e.g. "5 minutes").'),
  check_pop: z
    .boolean()
    .optional()
...
export const templateMandatoryShape = {
  ...
  lifetime: durationSchema.describe(
    'Mandatory certificate validity duration (e.g. "365d").',
  ),
  enabled: z
    .boolean()
