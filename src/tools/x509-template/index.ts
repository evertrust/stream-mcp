 ...
 * delete_template. Routes mounted at /api/v1/templates.
 *
 * Quirks honored:
 ...
 *  - `name` immutable; `lifetime` mandatory; `lifetime` accepts "365d".
...
const SPEC: ConfigSpec = {
  ...
  // Everything else round-trips (durations / eku.custom read back richer
  // but are accepted verbatim on PUT).
  stripFields: ['id'],
  putOnCollection: true,
  knowledgeRef: 'stream://knowledge/templates',
};
...
export function registerX509TemplateTools(
  ...
  registerCreateTool(server, client, SPEC, {
    description:
      ...
      '`lifetime` (duration like "365d"). `ku` (Key Usage) is required and ' +
    ...
    inputSchema: z.object({
      ...templateMandatoryShape,
      ...templateOptionalShape,
      // `ku` is optional in the body but the server requires at least one
      // KU value across the template; a template with no `ku` at all
      // fails. So `ku` is effectively mandatory on create.
