 ...
 *
 * Routes mounted at /api/v1/ssh/templates.
 *
 * Quirks honored:
...
const KNOWLEDGE_REF = 'stream://knowledge/ssh';
...
const durationSchema = z
  .string()
  .regex(DURATION_RE, 'Duration like "30 days" / "36 days" / "5 minutes".');
...
const optionalShape = {
  ...
  backdate: durationSchema
    .optional()
    .describe('Optional. Backdate validity start (duration string).'),
...
export function registerSshTemplateTools(
  ...
  registerCreateTool(server, client, SPEC, {
    description:
      ...
      '(duration string). Ask the user for each; do not infer or invent them ' +
    ...
    inputSchema: z.object({
      ...
      lifetime: durationSchema.describe(
        'MANDATORY. Certificate validity (duration string), e.g. "30 days".',
      ),
      ...optionalShape,
    }),
    buildPayload: (args) =>
  ...
  registerUpdateTool(server, client, SPEC, {
    ...
    inputSchema: z.object({
      ...
      lifetime: durationSchema
        .optional()
        .describe(
          'Optional on update. Certificate validity (duration string), e.g. ' +
