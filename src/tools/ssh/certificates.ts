...
// Stream's sort order is an enum whose wire values are
// CASE-SENSITIVE (Asc/Desc/KeyAsc/KeyDesc).
// The shared buildSearchPayload/buildSortedBy helper emits uppercase ASC/DESC,
// which the server rejects (400 SSH-CERT-002 "error.expected.validenumvalue"). We therefore build sortedBy here with the correct casing and
  ...
  query: z
    ...
    .describe(
      'SCQL (SSH dialect) filter, e.g. `status is valid`, ' +
        '`ca equals "example-ssh-ca"`, `type equals "USER"`. Empty/omitted defaults ' +
...
function registerSearch(server: McpServer, client: StreamClient): void {
  registerTool(
    ...
    async (args) => {
      ...
      // uppercase ASC/DESC) with the correctly-cased sort order wire value.
