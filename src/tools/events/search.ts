 ...
 * Endpoints:
 *   - POST /events/search            -> paginated event search results
...
// Sort order wire values are case-sensitive: Asc/KeyAsc => +1, Desc/KeyDesc => -1.
...
function registerSearch(server: McpServer, client: StreamClient): void {
  registerTool(
    ...
    async (args) => {
      ...
      // server apply an empty filter (match all).
