 ...
 *   POST   /api/v1/timestamping/signers          -> create (name unique; cert forced empty)
 ...
 *
 * Quirks honored:
 *   - certificate is rich-on-read (object) but write-only PEM; forced empty on
 ...
 *   - dn is mandatory while the signer has no certificate and is cleared by
...
const SIGNER_SPEC: ConfigSpec = {
  ...
  idField: 'name',
  immutableKeys: ['name'],
  // certificate: rich-on-read object / write-only PEM, forced empty on create and
  ...
  // specified"). When a certificate exists the GET returns no dn (the server
  // cleared it), so there is nothing to strip; and on cert-attach the server
  // clears dn before validation, so a carried-over dn is harmless.
  ...
  hash_algorithm: z
    ...
    .describe(
      'Signing hash algorithm. REQUIRED by the server on create for RSA ' +
        'keys (400 "Missing hash algorithm" otherwise); EC ' +
  ...
  registerCreateTool(server, client, SIGNER_SPEC, {
    ...
    inputSchema: z.object({
      ...
      on_expiration_triggers: z
        ...
        .optional()
        .describe(
          'Trigger names fired on the on_tsa_signer_expiration event. Each must ' +
            'reference an existing trigger runnable on that event.',
        ),
    }),
    buildPayload: (args) => {
