...
`lifetime` (a duration string), never `validity`.
...
| `lifetime`               | Duration string            | **Mandatory.** Cert validity. Accepts `"365d"` or `"365 days"`; reads back as `"365 days"`. NOT `validity`. |
| `backdate`               | Duration string            | Optional notBefore backdating (e.g. `"5 minutes"`, `"0 days"`).                                             |
...
| `subject`                | DN element[]               | DN/subject constraints. If present must be non-empty.                                                       |
...
`{ "type": <DN element type>, "mandatory": bool, "editable": bool, "value": str?, "regex": str?, "whitelist": [str]? }`.
...
DN element types: `CN`, `UID`, `SERIALNUMBER`, `SURNAME`, `GIVENNAME`, `T`,
...
`{ "type": <SAN type>, "min": int?, "max": int?, "regex": str? }`.
No duplicate `type`; `min/max >= 0` and `max >= min`; `regex` must compile and
be anchored (`^...$`).

SAN type values: `RFC822NAME`, `DNSNAME`, `URI`, `IPADDRESS`,
...
| `lifetime`           | Duration string  | **Mandatory.** Cert validity, e.g. `"30 days"`. NOT `validity`.                |
| `type`               | `USER` or `HOST` | Optional. If omitted, the enroll request supplies it (subject to CA override). |
| `backdate`           | Duration string  | Optional backdate of validity start.                                           |
...
- **Duration asymmetry**: send `"365d"`, read back `"365 days"`.
