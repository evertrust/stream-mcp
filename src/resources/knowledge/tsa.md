...
| `acceptedHashAlgorithms` | string[] | yes | **>=1** element; values from the hash algorithm list (below) |
...
| ------------- | --------------------------------------------- | ----------- | ------------------------------------------------------------------------------------------------------------------- |
| `name`        | string                                        | yes         | primary key                                                                                                         |
| `dn`          | string                                        | conditional | subject DN, e.g. `"CN=prod-tss"`. **Required when there is no certificate.** Server clears `dn` once a cert exists. |
| `privateKey`  | object                                        | yes         | see below                                                                                                           |
| `certificate` | string (PEM) on input / rich object on output | optional    | **forced to empty on create** — see lifecycle                                                                       |
...
| `hashAlgorithm` | string enum | optional | Hash algorithm; optional for EC keys; may be server-overridden to enforce compliance    |
...
1. `create_tsa_signer` with a `dn` and a `privateKey` (no certificate — it is forced to empty regardless of what you send):
   ```json
   {
     "name": "prod-tss",
     "dn": "CN=prod-tss",
...
Update merge is state-dependent: once a signer **has** a cert, the `certificate` and `privateKey` keystore+key are immutable on update (only `usePSS` and `hashAlgorithm` are taken from your body, and `dn` is cleared). To replace a cert/key you delete and recreate the signer.
...
| `timeout`     | string  | optional | Duration string, e.g. `"10 seconds"`, `"5 s"`, `"500 ms"`; client default 5s |
| `version`     | integer | optional | NTP protocol version                                                         |
| `maxStratum`  | integer | optional | sanity bound, 0..15                                                          |
| `maxOffset`   | string  | optional | Duration string, > 0                                                         |
...
The duration format is `<int> <unit>` (space optional): units `ms`, `s`, `m`, `h`, `d` (and long forms `milliseconds`, `seconds`, …). `delete_ntp_client` is guarded: 403 `NTP-005` if any authority references it (detail lists the TSA names).
