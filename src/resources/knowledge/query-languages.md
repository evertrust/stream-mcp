...
| `in [a,b,...]` / `not in [...]`             | id, string       | Value is (not) one of the literal array items.                                                               |
| `within [a,b,...]` / `not within [...]`     | string           | Case-insensitive substring match against any item of the array.                                              |
...
serial equals 00112233445566778899aabbccddeeff
...
- **Omit `fields` to get the full result object.** If you pass `fields`, you get exactly that projection; fields you did not request come back absent or null. Invalid field → `400 CERTIFICATE-002` (X509) / `400 SSH-CERT-002` (SSH) listing the usable fields.
...
→ `{ "items": [ {"_id":{"status":"valid"},"count":3000}, {"_id":{"status":"expired"},"count":1500}, {"_id":{"status":"revoked"},"count":500} ], "count": 5000 }`
...
`sortedBy` is an array of `{ "element": <field>, "order": <order> }` where `<order>` is `Asc`, `Desc`, `KeyAsc` or `KeyDesc`.
...
- Default when omitted: server default order (effectively ascending `id`).

### Pagination

| Field       | Type | Default                               | Notes                                                                                        |
| ----------- | ---- | ------------------------------------- | -------------------------------------------------------------------------------------------- |
| `pageIndex` | int  | `1`                                   | **1-based.** Any value `< 1` is coerced to `1`. Page n starts after (n-1)\*pageSize results. |
...
Every search response also returns `hasMore`, so you can page until `hasMore` is false.
...
→ `{ "results": [ … ], "pageIndex": 1, "pageSize": 2, "count": 1000000, "hasMore": true }`
