...
The accepted `type` values are exactly **`license`** and **`internal_monitor`**. (Other categories like SMTP/email or LDAP are managed as their own dedicated objects, not as system-configuration entries — do not pass a `type` the dictionary doesn't list; unknown type → 400.)
...
- `internal_monitor` (InternalMonitorConfiguration): **required** `cron` = a Quartz cron string (e.g. `"0 0 0 ? * * *"`); invalid → validation error. The server may round-trip extra whitespace on read (e.g. `"0 * * ? * * "`).

`upsert_system_configuration` is the only write path (no POST/DELETE). It looks up the existing entry of the same `type`: found → 200 full-replace reusing the prior id; not found → 201 create. `id` is server-assigned and ignored on writes. Requires `configuration:system:configuration:manage`.
...
- `throttle_duration` (optional) — a duration string matching `^([0-9]+) *(ms|...|days)$`, e.g. `"1 second"`, `"500 ms"`, `"5 minutes"`. Must be > 0.
...
- `get_license_info` — full `LicenseInfo`: `isValid`, `expiration` (ISO instant, omitted if perpetual), `version`, `buildTime`, `modules` (entitled module names, sorted), `libraries` (`{name, version}`), `releaseChannel`.
- `get_license_modules` — just the entitled module names. **No auth required** (plain action); returns an empty list if nothing entitled.

Module names map to product entitlements:
...
- `get_event` — one event by 24-hex ObjectId. Non-ObjectId → 400; unknown id → `EVT-003`.
...
- `list_event_integrity_reports` — all integrity reports, each re-verified on read (`status` may be server-overridden: `verified`, `running`, `eventIntegrityFailure`, `reportIntegrityFailure`, `unexpectedFailure`).

Event search result fields: `id`, `code`, `module`, `node`, `timestamp`, `status` (`success`/`failure`/`warning`), optional `details[]` (`{key,value}`), optional `seal` (read-only JWS). `seal` and `details` are never sortable.
