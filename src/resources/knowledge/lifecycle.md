...
`POST /api/v1/lifecycle/enroll` → **201** with the full certificate object (no permissions wrapper). CSR-based only.
...
- `template_overrides` — per-request overrides of the certificate template: `ku`, `eku`, `empty_extensions`, `crldps`, `aia`, `policy`, `path_len`, `lifetime` (duration string, e.g. `"365 days"`), `backdate`, `check_pop`, `extra_csr_extensions`. NOT overridable: `privateKeyUsagePeriod`, `removeBasicConstraints`, `qcStatement`.
...
`POST /api/v1/lifecycle/revoke` → **200** with the now-revoked certificate object (`revoked=true`, `revocationDate` + `revocationReason` set).
...
`reason` is **REQUIRED** by this tool (server-side it defaults to `unspecified`, but the MCP requires it for correct lifecycle hygiene). It is an RFC revocation reason string — exactly these 7 values (case-insensitive on input, echoed as the RFC string on output):
...
- `template` — object: `name` (existing, **enabled** template; disabled/missing → `404 SSH-TEMPLATE-003`) plus optional overrides `type`, `lifetime`, `backdate` (duration strings like `"12 hours"`). Each override is allowed only if the CA's `overridePermissions.{type,lifetime,backdate}` is true.
...
```json
{ "serial": "12345", "ca": "example-ssh-ca" }
```
