...
| `krlPolicy`           | yes                    | `{ validity, hardGeneration?, lazyGeneration? }`. `validity` (duration string, e.g. `"14 days"`) is required; the two cron fields are optional Quartz strings. |
...
| `lifetime`           | yes | Duration string (e.g. `"30 days"`). Mandatory — missing → `SSH-TEMPLATE-002` ("lifetime is mandatory").                     |
| `type`               | no  | `USER` or `HOST` (uppercase, case-sensitive). If omitted, the enroll request must supply the type (subject to CA override). |
| `backdate`           | no  | Duration string; backdates validity start.                                                                                  |
...
```
type equals "USER"
status is valid and ca equals "example-ssh-ca"
```
...
- By serial + CA: `{ "serial": "12345", "ca": "example-ssh-ca" }` — both required together. Serial is a decimal string. Providing only one of the pair → parse error. None present → parse error. All parse errors → 400 `LIFECYCLE-SSH-006`.
...
Lists, per ready CA, the templates the caller may use. Optional `permission` query param: `enroll` / `revoke` / `search` (case-sensitive lowercase, **default `search`**). For `enroll` it also requires the CA's `enroll=true`. Bad value → 400 enum-parse error. Response: `[ { "ca": "<name>", "templates": ["<name>", ...] }, ... ]`. Empty → 204.
