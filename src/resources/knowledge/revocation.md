...
`CRLInfo` fields (unset fields are omitted on the wire):
...
```jsonc
// update_crl_next_refresh({ ca: "example-ca", nextRefresh: "2026-12-31T00:00:00Z" })
// -> 200 { id, ca:"example-ca", type:"managed", number:298, ..., nextRefresh:"2026-12-31T00:00:00Z", size:0 }
```
...
`privateKey.hashAlgorithm` is a hash algorithm (`SHA256` / `SHA384` / `SHA512`, etc.; omitted for EdDSA). The server may normalize/enforce it to match the certificate.
...
Requires the **VA module** (without it, `enableOCSP` is cleared). `ocspSigner` is validated to reference an existing signer (else 400 `CA-002` (Invalid Certificate Authority), with a detail that names the missing signer). This is the reverse of the delete-guard: a signer referenced by any CA cannot be deleted (`OCSP-SIGNER-005`). Because the underlying CA update is full-replace, the tool follows **GET-strip-merge-PUT** on the CA object — do not hand-build a partial CA body.
...
// 2) CA update: set the CA `triggers.onCRLGeneration` += ["publish-to-prod-stream"]
```
